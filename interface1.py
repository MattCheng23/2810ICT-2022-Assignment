...
class Calendar:
    ...
    def __place_widgets(s):
        ...
        ttk.Button(bframe, text="YES", width=6, command=lambda: s._exit(True)).grid(row=0, column=0, sticky='ns',
                                                                                      padx=20)
...
if __name__ == '__main__':
    ...
    width, height = root.winfo_reqwidth() + 500, 500
    ...
    date_str_gain = lambda: [
        ...
        if date]
    tk.Button(root, text='date:', command=date_str_gain).place(x=0, y=0, relx=1 / 20, rely=1 / 6, relwidth=4 / 20,
                                                               relheigh=2 / 3)
    root.mainloop()